...
typedef void (*ble_lbs_alarm_write_handler_t) (ble_lbs_t * p_lbs, uint32_t new_state);

typedef struct
{
    ble_lbs_alarm_write_handler_t alarm_write_handler;                    /**< Event handler to be called when Alarm characteristic is written. */
} ble_lbs_init_t;
...
typedef struct ble_lbs_s
{
    ...
    ble_gatts_char_handles_t    alarm_char_handles;
    uint8_t                     uuid_type;
    uint16_t                    conn_handle;
    ble_lbs_alarm_write_handler_t alarm_write_handler;
} ble_lbs_t;
...
/**@brief Function for sending a alarm state notification.
 */
