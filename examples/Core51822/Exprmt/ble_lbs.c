...
static void on_write(ble_lbs_t * p_lbs, ble_evt_t * p_ble_evt)
{
	...
	{
		p_lbs->alarm_write_handler(p_lbs, *((uint32_t *)p_evt_write->data));
	}
}
...
/**@brief Function for adding the Alarm characteristic.
 *
 */
static uint32_t alarm_char_add(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ble_gatts_attr_t    attr_char_value;
...
uint32_t ble_lbs_init(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ...
    p_lbs->conn_handle       = BLE_CONN_HANDLE_INVALID;
    p_lbs->alarm_write_handler  =p_lbs_init->alarm_write_handler;
    ...
    err_code = alarm_char_add(p_lbs, p_lbs_init);
	if (err_code != NRF_SUCCESS)
	{
		return err_code;
	}
...
uint32_t ble_lbs_on_button_change(ble_lbs_t * p_lbs, uint8_t button_state)
{
    ...
    if (err_code == BLE_ERROR_GATTS_SYS_ATTR_MISSING){
    	err_code = sd_ble_gatts_sys_attr_set(p_lbs->conn_handle, NULL, 0);
    	if(err_code != NRF_SUCCESS){
    		return err_code;
    	}
    	err_code = sd_ble_gatts_hvx(p_lbs->conn_handle, &params);
...
uint32_t ble_lbs_on_alarm(ble_lbs_t * p_lbs, uint32_t alarm_state)
{
    ble_gatts_hvx_params_t params;
    uint16_t len = sizeof(alarm_state);
    uint32_t err_code;

    memset(&params, 0, sizeof(params));
    params.type = BLE_GATT_HVX_NOTIFICATION;
    params.handle = p_lbs->alarm_char_handles.value_handle;
    params.p_data = (uint8_t*)&alarm_state;
    params.p_len = &len;
